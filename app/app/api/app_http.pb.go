...
const OperationAppDailyReward = "/api.App/DailyReward"
...
func _App_DailyReward0_HTTP_Handler(srv AppHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in DailyRewardRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationAppDailyReward)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.DailyReward(ctx, req.(*DailyRewardRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*DailyRewardReply)
		return ctx.Result(200, reply)
	}
}
...
type AppHTTPClient interface {
	...
	DailyReward(ctx context.Context, req *DailyRewardRequest, opts ...http.CallOption) (rsp *DailyRewardReply, err error)

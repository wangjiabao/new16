...
func (a *AppService) DepositBiw(ctx context.Context, req *v1.DepositRequest) (*v1.DepositReply, error) {
	...
	if nil != err {
		...
		return &v1.DepositReply{}, err
	}

	addressBalance := make(map[string]string, 0)
	for _, v := range users {
		var (
		...
		for j := 0; j < 15; j++ {
			...
			tokenAddress := common.HexToAddress("0xB497819e28A8C92aD5bf8e3ba5f63D2b84999999")
		...
		addressBalance[v.Address] = bal.String()
	...
	price, err = getPrice()
	if err != nil {
		fmt.Println(err, "查询价格失败")
		return &v1.DepositReply{}, err
	}

	if 0 >= price {
		fmt.Println("价格0", price)
		return &v1.DepositReply{}, err
	}

	fmt.Println(price)
	err = a.ruc.ClearUserRaw(ctx)
	if err != nil {
		fmt.Println(err, "清空持币失败")
		return &v1.DepositReply{}, err
	}

	usersMap := make(map[int64]*biz.User, 0)
	for _, vUsers := range users {
		usersMap[vUsers.ID] = vUsers
	}

	for _, vUser := range users {
		if _, ok := addressBalance[vUser.Address]; !ok {
			fmt.Println("未查询到信息")
		}
		...
		raw.SetString(addressBalance[vUser.Address])

		// 除以 10^18 得到实际余额
		divisor := new(big.Float).SetFloat64(1e18)
		realBalance := new(big.Float).Quo(raw, divisor)
		// 转为 float64
		balanceRaw, _ := realBalance.Float64()
		tmpU := balanceRaw * price

		err = a.uuc.UpdateUserRaw(ctx, vUser.ID, tmpU, balanceRaw, usersMap)
		if err != nil {
...
func (a *AppService) AdminWithdrawEth(ctx context.Context, req *v1.AdminWithdrawEthRequest) (*v1.AdminWithdrawEthReply, error) {
	...
	for {
		...
		if "RAW" == withdraw.Type {
			tokenAddress = "0xB497819e28A8C92aD5bf8e3ba5f63D2b84999999"
...
func getTodayReward() (string, error) {
	...
	for j := 0; j < 15; j++ {
		...
		now := time.Now().UTC()
		startOfLastHour := time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.UTC)
		// 转换为 *big.Int
...
func getPrice() (float64, error) {
	...
	for _, urlTmp := range urls {
		...
		tokenAddress := common.HexToAddress("0x509b7Ac6Bc2dB85444e3a400273e3245DA5DF5b3") // LP Pair 地址
